    ...
    def get_chatbody_message(self, message):
        ...
        for key, value in jsonObjs.items():
            if 'message' == key:
                ...
                logger.info("Python进过一列标准化最后放到Chatboddy中message的数据为:%s"%chatbody_newMessage)
                break
        return jsonObjs
...
def chises_to_alnum(text_str):
    text_str = re.sub(u"(?P<value>[一二三四五六七八九十壹贰叁肆伍陆柒捌玖拾貮两百千万亿]+)", to_chinese_two, text_str)
    return text_str
...
def dispatcher(result, messageType):
    for method in method_list:
        ...
        logger.info("Python调用方法%s标准化数据:%s"%(eval(method).func_code.co_name, result))
